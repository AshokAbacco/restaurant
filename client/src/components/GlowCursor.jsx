...
const GlowCursor = ({
  ...
  console.log('GlowCursor mounted');
  propsRef.current = {
    color,
    secondaryColor,
    trailLength,
  ...
  useEffect(() => {
    ...
    // Track pointer on window, not the container — the container is
    // fixed+inset-0 with pointer-events:none so clicks pass through to
    // the app underneath. Listening on `container` alone never fires.
