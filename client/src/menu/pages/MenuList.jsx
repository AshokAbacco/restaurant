...
const FoodTypeDot = ({ foodType }) => {
  const border = foodType === "VEG" ? "border-[#3FA34D]" : foodType === "EGG" ? "border-[#E8873A]" : "border-[#EF5350]";
  const dot = foodType === "VEG" ? "bg-[#3FA34D]" : foodType === "EGG" ? "bg-[#E8873A]" : "bg-[#EF5350]";
  return (
...
const StatusPill = ({ status }) => {
  const styles = { ACTIVE: ui.badgeGreen, OUT_OF_STOCK: ui.badgeAmber, INACTIVE: ui.badgeGray, DELETED: ui.badgeRed };
...
const handleImportFile = async (e) => {
  ...
  return (
    <div className="space-y-6">
      ...
      {(canManage || canDelete) && (
        <div className={`${ui.card} p-4`}>
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div>
              <h3 className={`text-sm font-semibold ${ui.heading}`}>Bulk Import / Export</h3>
              <p className={`text-xs ${ui.faint} mt-0.5`}>Update many items at once using a CSV file</p>
            </div>
            <div className="flex items-center gap-2">
              <button onClick={() => setShowImportHelp(!showImportHelp)} className={`inline-flex items-center gap-1.5 ${ui.linkEdit} text-sm`}>
              ...
              {canDelete && (
                <button onClick={handleImportClick} disabled={importing} className={`${ui.btnSecondary} text-sm`}>
                  <FiUpload size={14} /> {importing ? "Importing..." : "Bulk Import"}
                </button>
              )}
              {canManage && (
                <button onClick={handleExport} disabled={exporting} className={`${ui.btnSecondary} text-sm`}>
          ...
          {showImportHelp && (
            <div className="mt-3 bg-[#3FA34D]/5 dark:bg-[#43B75A]/10 border border-[#3FA34D]/20 dark:border-[#43B75A]/25 rounded-xl px-4 py-4 text-xs text-[#1F2937] dark:text-white">
              <div className="flex items-center justify-between mb-3">
                <p className={`font-semibold ${ui.heading}`}>CSV column reference</p>
                <button
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className={`align-top ${ui.muted}`}>
                  {[
                    ["name", "Required", "The dish or drink's full name", "Chicken Biryani"],
                    ["sku", "Required", "A unique code for this item — no two items can share one", "BIR-001"],
                    ["categoryName", "Required", "Must exactly match an existing category name (see Categories tab)", "Biryani"],
                    ["sellingPrice", "Required", "Price the customer pays, numbers only", "320"],
                    ["costPrice", "Optional", "What it costs you to make — leave blank if unknown", "150"],
                    ["gstPercent", "Optional", "Tax percentage — leave blank to default to 0", "5"],
                    ["foodType", "Optional", "One of: VEG, NON_VEG, EGG — defaults to VEG if left blank", "NON_VEG"],
                    ["description", "Optional", "Short description shown to customers", "Fragrant basmati rice with spiced chicken"],
                  ].map((row) => (
                    <tr key={row[0]} className="border-t border-[#3FA34D]/10 dark:border-[#43B75A]/10">
                      <td className={`pr-4 py-1.5 font-medium ${ui.heading}`}>{row[0]}</td>
      ...
      {/* Filters */}
      <div className={`${ui.card} p-4 flex flex-col sm:flex-row flex-wrap gap-3`}>
        <div className="relative flex-1 min-w-[200px]">
          <FiSearch className={`absolute left-3 top-1/2 -translate-y-1/2 ${ui.faint}`} />
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by name, SKU, or barcode"
            className={`${ui.input} pl-10`} />
        </div>
        <select value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)} className={`${ui.input} sm:w-auto`}>
          {categoryOptions.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
        </select>
        <select value={subCategoryFilter} onChange={(e) => setSubCategoryFilter(e.target.value)} className={`${ui.input} sm:w-auto`}>
          {subCategoryOptions.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}
        </select>
        <select value={kitchenSectionFilter} onChange={(e) => setKitchenSectionFilter(e.target.value)} className={`${ui.input} sm:w-auto`}>
          {kitchenSectionOptions.map((k) => <option key={k.id} value={k.id}>{k.name}</option>)}
        </select>
        <select value={foodTypeFilter} onChange={(e) => setFoodTypeFilter(e.target.value)} className={`${ui.input} sm:w-auto`}>
        ...
        <select value={availabilityFilter} onChange={(e) => setAvailabilityFilter(e.target.value)} className={`${ui.input} sm:w-auto`}>
        ...
        {loading ? <Spinner /> : items.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {items.map((item) => (
                  <tr key={item.id} className="border-b border-[#E7EAE1] dark:border-[#262B24] last:border-b-0 hover:bg-[#F3F5EE]/60 dark:hover:bg-[#1E241C] transition-colors">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        <div className="w-11 h-11 rounded-xl bg-[#F3F5EE] dark:bg-[#1E241C] overflow-hidden flex-shrink-0 flex items-center justify-center shadow-sm">
                          {item.imageUrl ? <img src={item.imageUrl} alt="" className="w-full h-full object-cover" /> : <span className={`${ui.faint} text-lg`}>🍽️</span>}
                        </div>
                        <div className="min-w-0">
                          <div className="flex items-center gap-2">
                            <FoodTypeDot foodType={item.foodType} />
                            <span className={`font-medium truncate ${ui.heading}`}>{item.name}</span>
                          </div>
                          <span className={`text-xs ${ui.faint}`}>{item.sku}</span>
                        </div>
                      </div>
                    </td>
                    <td className={`px-4 py-3 ${ui.muted}`}>{item.category?.name || "—"}</td>
                    <td className={`px-4 py-3 ${ui.muted}`}>{item.kitchenSection?.name || "—"}</td>
                    <td className={`px-4 py-3 font-medium ${ui.heading}`}>₹{Number(item.sellingPrice).toFixed(2)}</td>
                    ...
                    <td className="px-4 py-3">
                      <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${item.isAvailable ? ui.badgeGreen : ui.badgeGray}`}>
                        {item.isAvailable ? "Available" : "Unavailable"}
                      </span>
                    </td>
                    <td className="px-4 py-3"><StatusPill status={item.status} /></td>
                    <td className="px-4 py-3 text-right">
                      <button onClick={() => setExtrasItem(item)} className={`inline-flex items-center gap-1 ${ui.linkEdit} text-xs`}>
                    ...
                    {(canManage || canDelete) && (
                      <td className="px-4 py-3 text-right">
                        <div className="flex justify-end gap-3">
                          {canManage && <button onClick={() => handleEdit(item)} className={ui.linkEdit}>Edit</button>}
                          {canDelete && <button onClick={() => setDeletingItem(item)} className={ui.linkDanger}>Delete</button>}
                        </div>
                      </td>
                    )}
                  </tr>
